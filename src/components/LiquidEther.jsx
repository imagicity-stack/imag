...
const THREE_SRC = 'https://cdn.jsdelivr.net/npm/three@0.164.0/build/three.min.js';
  ...
  return () => {
    ...
    if (!loaderPromise) {
      loaderPromise = new Promise((resolve, reject) => {
        ...
        script.async = true;
        script.addEventListener('load', () => resolve(window.THREE || null), { once: true });
  ...
  useEffect(() => {
    ...
    class MouseClass {
      constructor() {
        ...
        this._onWindowLeave = this.onWindowLeave.bind(this);
      ...
      init(container) {
        ...
        window.addEventListener('mouseout', this._onWindowLeave, false);
      }
      dispose() {
        ...
        window.removeEventListener('mouseout', this._onWindowLeave, false);
      }
      setCoords(x, y) {
        if (!this.container) return;
...
`;
    ...
    class WebGLManager {
      ...
      .catch((error) => {
        console.error('LiquidEther failed to load Three.js', error);
      });
